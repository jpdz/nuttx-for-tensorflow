...
TfLiteStatus MicroInterpreter::Invoke() {
  ...
  clock_t claasd = clock();
  auto dd = (int)((double)(claasd - start1) / CLOCKS_PER_SEC * 1000);
  int sum = 0;
  for (size_t i = 0; i < operators_->size(); ++i) {
    ...
    sum += gap; 
    TF_LITE_REPORT_ERROR(error_reporter_, "%s: %d msec\n",EnumNameBuiltinOperator(op_type),gap);
  }
  TF_LITE_REPORT_ERROR(error_reporter_, "Total Time for inference: %d msec\n",sum);
  return kTfLiteOk;
}
